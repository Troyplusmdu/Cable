...
namespace ACCSYS
{
	[DefaultProperty("TestProperty"),ToolboxData("<{0}:NavBar runat=server></{0}:NavBar>"), PersistChildren(false), Designer(typeof(ACCSYS.NavBarDesigner))]
		...
		protected override void Render(HtmlTextWriter output)
		{
			...
			foreach(NavBarBlock bl in this.Blocks) 
			{
				...
				if(bl.Items.Count>0 && allowed) 
				{
					...
					if(!bl.Expanded) 
						output.WriteAttribute("style", "display:none; overflow:auto");
					else 
						output.WriteAttribute("style", "display:block; overflow:auto");
